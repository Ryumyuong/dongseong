...
// ============== 대표 사례 확인 — 모바일 캐러셀 (무한루프, 1개/2개 반응형) ==============
(() => {
  const track   = document.getElementById('situationsTrack');
  const prevBtn = document.getElementById('situationsPrev');
  const nextBtn = document.getElementById('situationsNext');
  if (!track) return;
  ...
  // 다음 — 한 장씩 왼쪽으로, firstChild를 끝으로 보내 무한루프
  function next(fromTransformPx) {
  ...
  // 이전 — lastChild를 앞에 끼우고 0으로 애니메이션
  function prev(fromTransformPx) {
    if (isAnimating || !isMobile()) return;
    isAnimating = true;
    const s = step();
    setTransition(0);
    track.insertBefore(track.lastElementChild, track.firstElementChild);
    const startTx = (typeof fromTransformPx === 'number') ? (-s + fromTransformPx) : -s;
    track.style.transform = `translateX(${startTx}px)`;
    void track.offsetHeight;
    setTransition(ANIM_MS);
    track.style.transform = 'translateX(0)';
  ...
  function startTimer() {
    stopTimer();
    if (isMobile()) timer = setInterval(() => next(), AUTO_MS);
  }
  ...
  function onDown(e) {
    if (isAnimating || !isMobile()) return;
    dragging = true; startX = pointerX(e); delta = 0;
    setTransition(0); stopTimer();
  }
  function onMove(e) {
    if (!dragging) return;
    delta = pointerX(e) - startX;
  ...
  function onUp() {
    ...
    if (delta < -threshold) next(delta);
    else if (delta > threshold) prev(delta);
    else { setTransition(ANIM_MS); track.style.transform = 'translateX(0)'; }
    delta = 0; startX = 0; startTimer();
  }
  track.addEventListener('touchstart', onDown, { passive: true });
  track.addEventListener('touchmove',  onMove, { passive: true });
  track.addEventListener('touchend',   onUp);

  // 모드 전환 / 리사이즈 — 데스크탑에선 transform 제거(그리드 복원)
  function reset() {
    setTransition(0);
    track.style.transform = isMobile() ? 'translateX(0)' : '';
    if (isMobile()) startTimer(); else stopTimer();
  }
  window.addEventListener('resize', () => { if (!dragging && !isAnimating) reset(); });
  reset();
})();
